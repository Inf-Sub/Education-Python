...
# Напишите функцию, которая определит те элементы списка, у которых два соседних
...
def six_two(numbers: list) -> list:
    result = list()
    for index, element in enumerate(numbers):
        prev_index = index - 1
        next_index = index + 1 if index != len(numbers)-1 else 0
        if numbers[prev_index] < element and numbers[next_index] < element:
            result.append(element)
    return result

list_numbers = [1, 3, 3, 3, 5]
print(six_two(list_numbers))

list_numbers = [1, 5, 1, 6, 1]
print(six_two(list_numbers))
