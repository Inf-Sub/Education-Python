# №2.3[13]. Уставшие от необычно теплой зимы, жители решили узнать, действительно ли это самая длинная оттепель 
...
# период, в который среднесуточная температура ежедневно превышала 0 градусов Цельсия. Напишите программу, помогающую 
# синоптикам в работе.
...
if 1 <= count_days <= 100:
    for i in range(0, count_days):
        temp = int(input(f'Введите температуру за {i + 1} день: '))
        if temp > 0:
            current_days += 1
            if i == count_days - 1 and current_days > max_days:
